...
XSockets.BinaryMessage = (function () {
    function binaryMessage(message, arrayBuffer, cb) {
        ...
        if (message) {
            ...
            if (cb) cb.apply(this, [this.buffer, this.buffer.byteLength]);
        }
    }
    ...
    binaryMessage.prototype.extractMessage = function (message, cb) {
        throw "XSockets.Utils.extractBinaryMessage";
...
XSockets.WebSocket = (function () {
    function instance(url, controllers, parameters) {
        ...
        this.settings = XSockets.Utils.extend({
            ...
            queryString: function () {
                ...
                for (var key in _parameters) {
                    if (_parameters.hasOwnProperty(key)) {
                        str += key + "=" + encodeURIComponent(_parameters[key]) + "&";
                    }
                }
                ...
                onmessage: function (messageEvent) {
                    ...
                    if (typeof messageEvent.data === "string") {
                        ...
                        if (typeof (messageEvent.data) === "object") {
                            XSockets.Utils.extractBinaryMessage(messageEvent.data, function (message, binary) {
                              
                                self.dispatchMessage(message.topic, message, message.controller.toLowerCase(), binary);
                            });
        ...
        this.dispatchMessage = function (topic, message, controller,arrayBuffer) {
            self.controllerFactory.findByName(controller).forEach(function (match) {
                ...
                if (subscription) {
                    subscription.fire(message,arrayBuffer,topic, function (sub) {
...
XSockets.Subscription = (function () {
    function subscription(topic, delagate, count, completed) {
        ...
        this.fire = function (obj,arrayBuffer,cb) {
            this.delagate.apply(this, [obj, arrayBuffer || topic, topic]);
...
XSockets.Utils.extractBinaryMessage = (function (message, cb) {
    var byteArrayToLong = function (byteArray) {
        var value = 0;
        for (var i = byteArray.byteLength - 1; i >= 0; i--) {
            value = (value * 0xff) + byteArray[i];
        }
        return parseInt(value);
    };
    var header = new Uint8Array(message, 0, 8);
    ...
    var str = new Uint8Array(message, 8, payloadLength);
    var obj = (new XSockets.Message()).parse(String.fromCharCode.apply(null, new Uint16Array(str)));
    cb.apply(obj, [obj, buffer]);
    return this;
});